...
prepare_ilmn <- function(tumor_tabsep_file, normal_tabsep_file, tname,
    ...
    write.table(tbaf, tumor.baf.out, row.names=F, col.names=F, quote=F, sep="\t")
    tumor.lrr.out <- paste0(tname, "_mutantLogR.tab", sep="")
    write.table(tlrr, tumor.lrr.out, row.names=F, col.names=F, quote=F, sep="\t")
    ...
    write.table(nbaf, normal.baf.out, row.names=F, col.names=F, quote=F, sep="\t")

    normal.lrr.out <- paste0(tname, "_germlineBAF.tab", sep="")
    write.table(nlrr, normal.lrr.out, row.names=F, col.names=F, quote=F, sep="\t")
    # TODO
}
